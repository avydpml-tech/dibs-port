...
# Используем get_node вместо onready чтобы избежать краша в autoload
var _left_knob  = null
...
func _ready():
	# Ждём один кадр чтобы дерево нод точно было готово
	...
	# На Android с stretch mode 2d используем get_viewport().size
