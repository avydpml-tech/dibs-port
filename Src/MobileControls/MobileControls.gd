...
func _handle_touch(event: InputEventScreenTouch):
	...
	if event.pressed:
		# Кнопка выстрела — проверяем первой
		...
		# Левый стик — только нижняя левая четверть
		if not left_active and _in_left_zone(pos) and pos.y > screen.y * 0.5:
			left_active = true
		...
		# Правый стик — только нижняя правая четверть
		if not right_active and _in_right_zone(pos) and pos.y > screen.y * 0.5:
			right_active = true
		...
		# Тап по верхней части или центру = диалог
		...
		if fire_pressed and event.index != left_touch_index and event.index != right_touch_index:
			fire_pressed = false
			_release_action("ui_flashlight")

		if reload_pressed and event.index != left_touch_index and event.index != right_touch_index:
...
func _in_left_zone(pos: Vector2) -> bool:
	return pos.x < OS.get_screen_size().x * 0.35

func _in_right_zone(pos: Vector2) -> bool:
	return pos.x > OS.get_screen_size().x * 0.65
